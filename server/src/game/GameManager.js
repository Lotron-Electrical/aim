...
export class GameManager {
  ...
  startBattle(roomId) {
    ...
    battle.interval = setInterval(
      () => this._runTurn(roomId),
      BATTLE.TURN_INTERVAL_MS,
    );
  }
  ...
  _runTurn(roomId) {
    ...
    // Select actions
    const { action: action1, log: log1 } = selectAction(
      bot1,
      bot2,
      battle.turn,
    );
    const { action: action2, log: log2 } = selectAction(
      bot2,
      bot1,
      battle.turn,
    );
    ...
    let burstResult1 = null,
      burstResult2 = null;
    if (bot1.burstMode) {
      bot1.burstMode = false;
      const { action: extra } = selectAction(bot1, bot2, battle.turn);
      burstResult1 = resolveTurn(
        bot1,
        bot2,
        extra,
        { type: "rest", module: "NONE", behavior: null },
        battle.turn,
      );
    }
    if (bot2.burstMode) {
