...
// ============================================================
import { MODULES, ENERGY_REGEN, BATTLE, STAT_RANGES, TOTAL_TRAINING_POINTS, MAX_MODULE_LEVEL } from './constants.js';
...
export function validateBotConfig(config) {
  if (!config.name || typeof config.name !== 'string' || config.name.length < 1 || config.name.length > 20) {
  ...
  for (const mod of Object.keys(MODULES)) {
    ...
    if (typeof level !== 'number' || level < 0 || level > MAX_MODULE_LEVEL || !Number.isInteger(level)) {
  ...
  if (total !== TOTAL_TRAINING_POINTS) {
    return { valid: false, error: `Must allocate exactly ${TOTAL_TRAINING_POINTS} points (got ${total})` };
  }
...
export function selectAction(bot, enemy, turnNumber) {
  ...
  if (tacticsLevel >= 1 && enemy.actionHistory.length >= 2) {
    ...
    if (dominant[1] >= 2) {
      ...
      log.push(`TACTICS.PatternRecognition detected: enemy favors ${enemyPattern} (${dominant[1]}/${last3.length} turns)`);
    }
  }
  ...
  if (tacticsLevel >= 4) {
    const weakest = Object.entries(enemy.modules).sort((a, b) => a[1] - b[1])[0];
    if (weakest[1] <= 1) {
      targetWeakness = weakest[0];
      log.push(`TACTICS.PriorityTargeting: enemy weak in ${targetWeakness} (level ${weakest[1]})`);
    }
  }
  ...
  for (const [modName, behaviors] of Object.entries(available)) {
    ...
    for (const behavior of behaviors) {
      ...
      if (enemyPattern === 'DEFENSE' && behavior.name === 'Precision Strike') score += 25;
      if (enemyPattern === 'DEFENSE' && behavior.name === 'Energy Drain') score += 20;
      ...
      if (bot.energy > bot.maxEnergy * 0.7 && behavior.energyCost >= 25) score += 10;
  ...
  const confidence = clamp(Math.round((chosen.score / (candidates[0].score + 20)) * 100), 30, 99);

  log.push(`Decision: ${chosen.module}.${chosen.behavior.name} -- confidence: ${confidence}%`);

  return {
    action: { type: 'behavior', module: chosen.module, behavior: chosen.behavior },
    log,
  };
}
...
export function resolveTurn(bot1, bot2, action1, action2, turnNumber) {
  const results = { turn: turnNumber, bot1: { log: [], damage: 0 }, bot2: { log: [], damage: 0 } };
  ...
  if (action1.behavior) bot1.actionHistory.push({ module: action1.module, name: action1.behavior.name });
  if (action2.behavior) bot2.actionHistory.push({ module: action2.module, name: action2.behavior.name });
  ...
  results.bot1State = { hp: bot1.hp, maxHp: bot1.maxHp, energy: bot1.energy, maxEnergy: bot1.maxEnergy };
  results.bot2State = { hp: bot2.hp, maxHp: bot2.maxHp, energy: bot2.energy, maxEnergy: bot2.maxEnergy };
...
function executeAction(attacker, defender, action, attackerResult, defenderResult, turnNumber) {
  if (action.type === 'rest') {
  ...
  if (modName === 'ATTACK') {
    ...
    if (defender.counterAttack && totalDamage > 0) {
      ...
      defenderResult.log.push(`Counter-Attack: dealt ${counterDmg} damage back!`);
    }
  ...
  // --- DEFENSE behaviors ---
    ...
    if (b.name === 'Shield Wall') {
      ...
      attackerResult.log.push(`Shield Wall: blocking ${Math.round(b.damageReduction * 100)}% damage`);
    }
    else if (b.name === 'Dodge Matrix') {
      attacker.dodgeActive = true;
      attacker.dodgeChance = b.dodgeChance;
      attackerResult.log.push(`Dodge Matrix: ${Math.round(b.dodgeChance * 100)}% evasion active`);
    }
    else if (b.name === 'Counter-Attack') {
      attacker.shieldActive = true;
      attacker.shieldReduction = b.damageReduction;
      attacker.counterAttack = { counterDamage: b.counterDamage };
      attackerResult.log.push(`Counter-Attack: blocking ${Math.round(b.damageReduction * 100)}% + counter ready`);
    }
    else if (b.name === 'Regeneration') {
      attacker.regenRemaining = b.duration;
      attacker.regenPerTurn = b.healPerTurn;
      attackerResult.log.push(`Regeneration: +${b.healPerTurn} HP/turn for ${b.duration} turns`);
    }
      ...
      attacker.shieldActive = true;
      attacker.shieldReduction = b.damageReduction;
      attackerResult.log.push(`Fortress Mode: ${Math.round(b.damageReduction * 100)}% block, attacks locked`);
    }
  }
  ...
  // --- TACTICS behaviors ---
    ...
    if (b.name === 'Feint') {
      ...
      if (Math.random() < b.disruptChance) {
        defenderResult.log.push('Feint: disrupted! Next action accuracy reduced');
      ...
      if (defender.actionHistory.length >= 1) {
        const lastAction = defender.actionHistory[defender.actionHistory.length - 1];
        const action2 = defender.actionHistory[defender.actionHistory.length - 2];
        if (action2 && lastAction.module === action2.module) {
          defender.hp -= b.counterDamage;
          attackerResult.log.push(`Predict & Parry: predicted ${lastAction.module}! Dealt ${b.counterDamage} damage`);
        } else {
  ...
  // --- OVERCLOCK behaviors ---
    ...
    if (b.name === 'Burst Mode') {
      ...
      attackerResult.log.push('Burst Mode: ACTIVATED -- double action next turn');
    }
      ...
      const sorted = Object.entries(defender.modules).sort((a, b) => b[1] - a[1]);
      ...
      if (upgradeable.length > 0) {
        ...
        attackerResult.log.push(`Evolve: ${target} upgraded to level ${attacker.modules[target]}!`);
      } else {
      ...
      if (hit) {
        ...
        attacker.hp -= b.selfDamage;
        attackerResult.log.push(`Singularity: BACKFIRED! Took ${b.selfDamage} self-damage`);
      }
    }
  }
}
...
export function runFullBattle(config1, config2) {
  ...
  return { turns, finalState: { bot1: { hp: bot1.hp, maxHp: bot1.maxHp }, bot2: { hp: bot2.hp, maxHp: bot2.maxHp } } };
}
